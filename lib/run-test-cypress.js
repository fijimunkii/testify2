...
module.exports = async (options) => {
  ...
  await new Promise((resolve, reject) => {
    const cmd = 'docker run'+
      ...
      (options.skip?` -e CYPRESS_ignore_test_files=*${options.skip}*.js`:'')+
      ' --name='+name+
    ...
    test.on('close', async (exitCode, signal) => {
      ...
      if (exitCode || !output) {
        await fs.readdirAsync(path.join(tmpdir,'cypress/screenshots'))
          .then(screenshots => Promise.all(screenshots.map(filename =>
            fsExtra.copy(path.join(tmpdir,'cypress','screenshots',filename),
              path.join(options.logDir,filename)))))
          .catch(() => { /* path doesnt exist - no screenshots */ });
        await fs.readdirAsync(path.join(tmpdir,'cypress/videos'))
          .then(videos => Promise.all(videos.map(filename =>
            fsExtra.copy(path.join(tmpdir,'cypress','videos',filename),
              path.join(options.logDir,filename)))))
          .catch(() => { /* path doesnt exist - no videos */ });
