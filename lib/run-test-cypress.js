...
module.exports = async (options) => {
  ...
  await fs.symlinkAsync(path.join(env.get('LOG_DIR'),'screenshots'),path.join(tmpdir,'cypress/screenshots'),'dir');
  await new Promise((resolve, reject) => {
    ...
    test.stdout.on('data', data => { options.res.write(String(data)); });
    ...
    test.on('close', async (exitCode, signal) => {
      ...
      killTest();
      await fs.writeFileAsync(path.join(env.get('LOG_DIR'),'test.log'),output);
