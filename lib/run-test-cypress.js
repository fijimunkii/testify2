...
module.exports = async (options) => {
  const name = `${options.project}${+new Date()}`;
  ...
  await new Promise((resolve, reject) => {
    https.get(options.artifacts, res => {
  ...
  await sendMessage(`Testifying ${options.targetUrl}`);
  await new Promise((resolve, reject) => {
    const cmd = 'docker run'+
      ' -v '+tmpdir+'/cypress:/root/cypress'+
      ' -v '+options.logdir+':/root/logs'+
      ' -v '+path.join('/home/ubuntu/cert/',options.project)+':/root/cert-proxy/cert'+
